// import poster from "../assets/CPoster.png";
import { useState, useEffect, useRef } from "react";
import { comedy } from "../api/api";
...
function Comedy() {
  ...
  const scrollright = () => {
    ...
    setshowleft(true);
    setTimeout(() => {
      if (slider.current.scrollLeft + slider.current.clientWidth >= slider.current.scrollWidth -5 ) {
        setshowright(false);
      }
    }, 500);
  };
  ...
  useEffect(() => {
    comedy().then(data => { setcomedys(data) }
    )
  }, []);
